...
df = df.withColumn('Beneficios', df['monto_F_ABS'] - df['pro_Costo_del_articulo'])
df = df.withColumn('Perdidas', when(df['pro_Costo_del_articulo'] - df['monto_F_ABS'] < 0, 0).otherwise(df['pro_Costo_del_articulo'] - df['monto_F_ABS']))
df = df.withColumn('Rentabilidad', (df['Beneficios'] / df['monto_F_ABS']) * 100)
df = df.withColumn('Ingresos_Generados', df['monto_F_ABS'] - df['tranc_Descuento_total'])
df = df.withColumn('Margen_de_Beneficios', (df['Beneficios'] / df['Ingresos_Generados']) * 100)
df = df.withColumn('Monto_Total_con_Descuento', df['monto_F_ABS'] - df['tranc_Descuento_total'])
